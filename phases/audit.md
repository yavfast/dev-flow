...
8. **Knowledge scaling — proposed** (executed by the [skill phase](skill.md) only after confirmation): `split-planned` / `split-required` for a skill file over `full_read_limit` / `soft_split` (a new file in the same domain, ids unchanged); `knowledge-overload` per domain (entries over `activation_budget`) — advisory for skills.
...
7. **Documentation scaling** — mechanical measurements per [Docs Scaling](../references/docs-scaling.md) (thresholds and service-section list live there; none of these asks anyone to count by hand). The heading measurements skip fenced content — a `## …` line inside a code fence is a sample the document shows, not a section it has:
...
⚠️ Flagged (evidence insufficient — left unchanged)
   ...
   • Rules: duplicate anchor {#<id>} in <file-A>, <file-B> / alias <id> → <moved_to> target missing / category <name> has no applies_to (always-on; hint: <selector>) / skill-candidate: <category> <selector> — <n> rules <id…> (agent's call)
