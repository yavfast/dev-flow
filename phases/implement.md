...
- [ ] Contested technology forks are resolved (consensus + rationale) or recorded as open decisions with a resolution trigger (see [Interview Mode](../references/interview-mode.md))
...
**Intent check.** Re-read the task file's `## Intent` (goal / target state / expected result) before coding, and hold each material implementation decision against it: *does this option serve the recorded goal?* If executing the plan as written would not achieve the recorded goal — or the letter and the goal diverge — stop and surface the conflict (marked options + recommendation) instead of silently following either. See [Task Intent](../references/task-intent.md).
...
**Reuse check (advisory).** Before writing a new function, class, util, or shared constant, search for an existing one first — the `docs/_framework.md` shared-utilities map, `.dev_flow/skills/`, then a symbol/behaviour search of the codebase (delegate a wide search, keep the conclusion). Call an exact match; extend a near-match rather than fork it (unless the two cases change for different reasons); escalate if reuse needs a contract change. When writing new code, leave a reuse seam only for a real near-term consumer — speculative generality defaults to `drop + record`. The code-altitude counterpart of the concept [Reuse Check](concept.md#reuse-check). See [Code Reuse](../references/code-reuse.md).
