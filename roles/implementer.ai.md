...
role Implementer {
  ...
  responsibilities:
    ...
    - "Reuse existing code — search for an existing function/class before creating a new one; build for reuse only with a real near-term consumer (references/code-reuse.md)"
  ...
  rules:
    ...
    - "MUST search for an existing function/class/util before creating a new one (docs/_framework.md map, .dev_flow/skills/, codebase symbol/behaviour search); call an exact match, extend a near-match rather than fork it, escalate if reuse needs a contract change (references/code-reuse.md)"
    - "MUST NOT pre-abstract for a speculative consumer (YAGNI) — a reuse seam needs a real near-term consumer; resist over-DRY (do not fold look-alikes that change for different reasons)"
  ...
  workflow:
    ...
    - "Reuse-check: before writing a new function/class, search for an existing one (framework map, skills, codebase) — call/extend it rather than re-implement (references/code-reuse.md)"
