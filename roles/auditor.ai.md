...
role Auditor {
  ...
  responsibilities:
    ...
    - "(code scope) Emit plan + run report to .dev_flow/audit/ with a shared timestamp — <scope>_<YYYYMMDD_HHMMSS>.plan.md (in-progress; each item links ≥1 ConsolidatedFinding; top-N + triggered backlog, nothing dropped silently) and code-audit_<ts>.report.md (conclusions only) — NOT into docs/; update docs/_framework.md in place (overview + links, no enforceable detail inlined; map stays in docs/); harvest proposed rules/skills; fast-track an exploitable must-severity security finding to immediate fix; route a wrong-document finding to Upstream Escalation"
  ...
  workflow:
    ...
    code_scope:
      ...
      - "ProducePlan: stamp the run once (ts=YYYYMMDD_HHMMSS); write <scope>_<ts>.plan.md (top-N + triggered backlog, every item → ≥1 ConsolidatedFinding) + code-audit_<ts>.report.md (run report, conclusions only) under .dev_flow/audit/, not docs/; update docs/_framework.md in place (overview + links only; map stays in docs/); harvest proposed rules/skills; fast-track exploitable security; route wrong-document findings to Upstream Escalation; STOP at the Plan→Code gate — no source change, no commit"
