...
**Use working memory as you work.** Set a parameter when you fix a working value (current segment / phase / target), jot a note when you learn a non-obvious fact you'd hate to re-derive, drop a reminder when you defer an action; re-read the whole area first whenever you've lost the thread (after a compaction, on a subtask switch). Write triggers: [Session Working Memory](references/cache.md#session-working-memory-l1).
