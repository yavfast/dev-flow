...
**First, the coverage rung** ([Evidence Discipline](evidence-discipline.md)) — *covered* (name the covering artifact) → *extend* → *create* → *needs-evidence*, choosing the **narrowest durable owner** from `rule` · `skill` · `drop` · `none` (already covered) · `needs-evidence`: `rule` for any lesson from an incident (constraint, regularity, adaptation), `skill` only for knowledge that is not an artifact constraint or for the consolidated form of an accumulated rule cluster ([Knowledge Scaling](knowledge-scaling.md)), `drop` for a narrow one-off; defaulting to `skill` is forbidden. Directive, not blocking: record the reason and proceed. On *create*, open a dormant intervention-ledger entry (`lesson`/`artifact`/`owner_kind`/`problem_class`/`trigger`); a ledger failure skips the entry, never the write. Then:

- A **reusable constraint or convention** → write a [rule](../phases/rule.md) in the unit form of [Knowledge Scaling](knowledge-scaling.md) — directive in the heading (within `directive_max` — split, never truncate), category selector inherited or narrowed (never widened), body within `unit_body_max` (narrative stays in the task file), provenance = the task file, digest and index updated in the same edit. Auto-written rules default to **`should`**; a `prefer` is fine. **Never auto-write a `must`** — a `must` blocks future code, so it is treated as doubtful (below).
