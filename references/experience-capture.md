...
**First, the coverage rung** ([Evidence Discipline](evidence-discipline.md)) — *covered* (name the covering artifact) → *extend* → *create* → *needs-evidence*, choosing the **narrowest durable owner** from `rule` · `skill` · `drop` · `none` (already covered) · `needs-evidence`: `rule` for a constraint, `skill` for a procedure, `drop` for a narrow one-off; defaulting to `skill` is forbidden. The rule/skill discriminator is the kind test of [Knowledge Scaling](knowledge-scaling.md): a reviewer can check a rule from the artifact alone, a skill only from how the work was done; undeterminable → the rung's owner stands, noted `unobserved`. Directive, not blocking: record the reason and proceed. On *create*, open a dormant intervention-ledger entry (`lesson`/`artifact`/`owner_kind`/`problem_class`/`trigger`); a ledger failure skips the entry, never the write. Then:

- A **reusable constraint or convention** → write a [rule](../phases/rule.md) in the unit form of [Knowledge Scaling](knowledge-scaling.md) — directive in the heading (within `directive_max` — split or reclassify, never truncate), category selector inherited or narrowed (never widened), body within `unit_body_max` (narrative stays in the task file), provenance = the task file, digest and index updated in the same edit. Auto-written rules default to **`should`**; a `prefer` is fine. **Never auto-write a `must`** — a `must` blocks future code, so it is treated as doubtful (below).
